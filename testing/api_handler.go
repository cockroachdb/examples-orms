...
func (apiHandler) ping(app application) error {
	...
	if body, name := strings.TrimSpace(string(b)), app.name(); body != name {
		return errors.Errorf("ping response %s != expected %s", body, name)
	}
	return err
}
