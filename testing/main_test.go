...
// function that terminals that process.
func initORMApp(app application, dbURL *url.URL) (func() error, error) {
	...
	if err := cmd.Start(); err != nil {
		return killCmd, fmt.Errorf("command %s failed to start: args=%s", cmd.Args, err)
	}
	...
	for waited := time.Duration(0); ; waited += waitDelay {
		if processState := cmd.ProcessState; processState != nil && processState.Exited() {
			return killCmd, fmt.Errorf("command %s exited: %v", cmd.Args, cmd.Wait())
		}
		if err := (apiHandler{}).ping(app.name()); err == nil || waited > maxWait {
			return killCmd, err
		}
